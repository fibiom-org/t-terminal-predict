...
export function MarketScreen({ pair, onBack }: Props): React.ReactElement {
  ...
  const load = useCallback(
    async (range: number): Promise<void> => {
      ...
      if (errs.length) setError(errs.map((e) => String(e.reason).split('\n')[0]).join(' · '));
    ...
    >
      <Box flexDirection="column">
        <Box justifyContent="space-between">
          <Text bold>
            Market · <Text color="cyan">{pair.label}</Text>
          </Text>
        ...
        <Box marginTop={1} flexDirection="column" borderStyle="round" borderColor="gray" paddingX={1}>
          ...
          {pool ? (
            ...
            <Text dimColor>pool data unavailable</Text>
          )}
        </Box>
